...
const WEBAUTHN = require("./challenge-commands/webauthn.js");
COMMANDS.WEBAUTHN_SETUP = WEBAUTHN.WEBAUTHN_SETUP;
COMMANDS.WEBAUTHN_VALIDATE = WEBAUTHN.WEBAUTHN_VALIDATE;
COMMANDS.WEBAUTHN_MFA_CHECK = WEBAUTHN.WEBAUTHN_MFA_CHECK;
COMMANDS.WEBAUTHN_REVOKE = WEBAUTHN.WEBAUTHN_REVOKE;
COMMANDS.WEBAUTHN_WRITE_BLOCK = WEBAUTHN.WEBAUTHN_WRITE_BLOCK;
COMMANDS.WEBAUTHN_REMOVE_BLOCK = WEBAUTHN.WEBAUTHN_REMOVE_BLOCK;
...
var handleCommand = function (Env, req, res) {
    ...
    try {
        COMMANDS[command](Env, body, function (err, extra) {
            if (err) {
            ...
            Challenge.write(Env, txid, JSON.stringify(copy), function (err) {
                ...
                // respond with challenge parameters; commands may provide extra data
                // (e.g. WebAuthn registration/authentication options) via the second cb arg
                return void res.status(200).json(Object.assign({
                    txid: txid,
                    date: date,
                }, extra || {}));
            });
...
var handleResponse = function (Env, req, res) {
    ...
    Challenge.read(Env, txid, function (err, text) {
        ...
        // Commands may declare stage2Extensions: an array of additional parameter names they
        // accept in the Stage 2 request (beyond sig and txid). These extensions are NOT part
        // of the signed body; their integrity is guaranteed by the command's own verification
        // (e.g. WebAuthn attestation/assertion responses are bound to the stored challenge).
        var allowedExtras = (COMMANDS[json.command] && COMMANDS[json.command].stage2Extensions) || [];
        var invalidKeys = Object.keys(body).filter(k => k !== 'sig' && k !== 'txid' && !allowedExtras.includes(k));
        ...
        // Some commands inject server-generated fields into body during Stage 1
        // (e.g. webauthnChallenge). Those fields are stored in the challenge but were
        // never sent to the client, so the client's signature covers a body WITHOUT them.
        // Strip them before signature verification; they are still in `json` for .complete().
        var serverExts = (COMMANDS[json.command] && COMMANDS[json.command].stage1ServerExtensions) || [];
        var signableBody = Object.assign({}, json);
        serverExts.forEach(k => { delete signableBody[k]; });
        ...
        try {
            u8_toVerify = Util.decodeUTF8(JSON.stringify(signableBody));
        ...
        // Merge any declared stage2 extension params into the stored body so .complete() can read them.
        // This happens after signature verification so the core challenge body is already authenticated.
        allowedExtras.forEach(k => { if (body[k] !== undefined) { json[k] = body[k]; } });
