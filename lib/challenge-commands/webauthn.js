...
const WEBAUTHN_SETUP = Commands.WEBAUTHN_SETUP = function (Env, body, cb) {
    ...
    MFA.read(Env, publicKey, function (err, existing) {
        ...
        var options = generateRegistrationOptions({
            rpName: rp.rpName,
            rpID: rp.rpId,
        ...
        // Return registrationOptions to the client so it can call navigator.credentials.create().
        // The attestationResponse from the authenticator then comes back in Stage 2.
        cb(void 0, { registrationOptions: options });
    });
};

// stage2Extensions declares which extra Stage 2 params this command accepts beyond {sig, txid}.
// attestationResponse is generated by the authenticator after Stage 1 and provided by the client.
...
const WEBAUTHN_VALIDATE = Commands.WEBAUTHN_VALIDATE = function (Env, body, cb) {
    ...
    readMFA(Env, publicKey, function (err, mfaData) {
        ...
        var options = generateAuthenticationOptions({
            rpID: rp.rpId,
            challenge: Buffer.from(challenge, 'base64url'),
            allowCredentials,
            userVerification: 'preferred',
